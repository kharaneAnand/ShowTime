...
const AddShows = () => {
  ...
  const handelDateTimeAdd = () => {
    ...
    if (dateTimeInputRef.current) {
      dateTimeInputRef.current.focus();
      dateTimeInputRef.current.showPicker?.(); // reopen picker
    }
  };
                ...
                <div key={date}>
                  ...
                  <div className="flex flex-wrap gap-2 mt-1">
                    {times.map((time) => (
                      <div
                        key={time}
                        className="flex items-center gap-2 bg-gray-700 text-white text-sm px-3 py-1 rounded-full"
                      >
                        <span>{time}</span>
                        <button
                          onClick={() => handelRemoveTime(date, time)}
